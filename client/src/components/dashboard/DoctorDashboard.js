...
export default function DoctorDashboard() {
	...
	const getPatientsTreatedCount = async () => {
		const response = await axios.get(`https://hms-backend-dvsa.onrender.com/count/patients/treated`,
			{
				headers: {
					authorization: `Bearer ${localStorage.getItem("token")}`
				}
	...
	const getPrescription = async () => {
        ...
        let response = await axios.post(`https://hms-backend-dvsa.onrender.com/prescriptions`,{},
            {
                headers: {
                    authorization: `Bearer ${localStorage.getItem("token")}`
                }
